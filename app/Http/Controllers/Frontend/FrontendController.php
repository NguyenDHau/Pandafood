...
class FrontendController extends Controller
{
    ...
    public function getDetail($product_id){
        ...
        $data['shop'] = User::find($data['title']['user_id']);
//        dd($data['shop']);
        $data['product'] = DB::table('pdf_products')
            ->join('pdf_categories','pdf_products.category_id','=','pdf_categories.category_id')
            ->select('pdf_products.*','pdf_categories.category_id','pdf_categories.category_title','pdf_categories.category_slug', 'pdf_products.user_id')

...
class UserController extends Controller
{
    ...
    public function getShop() {
        $data['userlist'] = DB::table('pdf_users')->join('pdf_groups','pdf_users.group_id','=','pdf_groups.group_id')->whereNotNull('pdf_users.user_shop_name')->where('pdf_users.group_id', '=', 2)->orderBy('user_id','desc')->get();
        return view('admincp.user.shop',$data);
    }

    public function acceptShop($id){
        $user = User::find($id);
        $user->group_id = 3;
        $user->save();
